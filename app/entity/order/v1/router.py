...
def list_orders(pageable: PageRequestSchema = Depends(),
                order_service: OrderService = Depends(OrderService)) -> PageResponseSchema:
    data, total_count = order_service.get_paged_orders(pageable)
    response = PageResponseSchema(
        total_count=total_count,
        page_size=pageable.size,
        data=[OrderOut.model_validate(orm) for orm in data]
    )
    return response

...
async def sql_error_handler(request, exc):
    ...
    return JSONResponse(
        status_code=HTTP_500_INTERNAL_SERVER_ERROR,
        content=jsonable_encoder(_build_error_dict('Internal Server Error', 'Database error occurred')),
    )


async def data_not_found_error_handler(request, exc):
    return JSONResponse(
        status_code=HTTP_404_NOT_FOUND, content=jsonable_encoder(_build_error_dict('Not Found Error', str(exc)))
    )

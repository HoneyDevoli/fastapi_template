...
class TestOrderRouter:
    ...
    def test_create_order_error(self, client: TestClient):
        ...
        assert response.json()['errors'] == [
            {'msg': 'Input should be a valid number', 'source': 'body/price', 'title': 'Request Validation Error'},
            {
    ...
    def test_get_order_error(self, client: TestClient):
        ...
        assert response.json()['errors'] == [
            {'title': 'Not Found Error', 'msg': 'No row was found when one was required'}
        ]
    ...
    def test_list_order_error(self, client: TestClient):
        ...
        assert response.status_code == 422
        assert response.json()['errors'] == [
            {'title': 'Attribute Error', 'msg': "type object 'OrderOrm' has no attribute 'test'"}
        ]
